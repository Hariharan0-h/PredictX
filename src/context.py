...
Simulates ambient_temp_ext and factory_load, merges with internal features,
runs ablation (internal-only vs fused) to prove external context adds lift.
Input:  data/features_internal.parquet
Output: data/features_fused.parquet
...
def run_cv(X: pd.DataFrame, y: pd.Series, label: str) -> float:
    ...
    mean_f1 = scores.mean()
    print(f"  [{label}] Macro F1: {scores.round(4)} → mean={mean_f1:.4f} ± {scores.std():.4f}")
...
def main() -> pd.DataFrame:
    ...
    print(f"Loaded internal features: {df_internal.shape}")
    ...
    print(f"Saved fused features → {OUTPUT_PATH}")
    ...
    print(f"\nMacro F1 lift from external context: {delta:+.4f}")

    ablation = {
        "internal_only_macro_f1": f1_internal,
        "fused_macro_f1": f1_fused,
        "delta": delta,
    }
    with open(RESULTS_DIR / "ablation.json", "w") as f:
        json.dump(ablation, f, indent=2)
    print(f"Saved → {RESULTS_DIR / 'ablation.json'}")
